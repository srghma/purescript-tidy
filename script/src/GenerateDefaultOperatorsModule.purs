...
import Data.Array (mapWithIndex, foldMap)
...
main = runAff_ (either throwException mempty) do
  ...
  let tmpPath = "/tmp/purs-tidy-generate-default-operators-pazdhI"
  ...
  (pursFiles :: Array String) <- getPursFiles 0 (tmpPath <> "/.spago")

  let genCmdFile = Path.concat [ cwdPath, "bin", "index.js" ]
  let genCmdArgs = [ "generate-operators" ] <> Array.take 5 pursFiles
